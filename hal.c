...
static const PIN_Config radioPinTable[] = {
	Board_SX_CSN      | PIN_GPIO_OUTPUT_EN | PIN_GPIO_HIGH | PIN_PUSHPULL,
...
static const PIN_Config radioSleepPinTable[] = {
	Board_SX_CSN      | PIN_INPUT_EN | PIN_NOPULL,
	Board_SX_RF_CTRL1 | PIN_INPUT_EN | PIN_NOPULL,
	Board_SX_RF_CTRL2 | PIN_INPUT_EN | PIN_NOPULL,
	Board_SX_RESET    | PIN_INPUT_EN | PIN_NOPULL,
	PIN_TERMINATE
};

// PIN radio interrupts config
//static const PIN_Config intruptPinTable[] = {
//	Board_SX_DIO0       | PIN_INPUT_EN | PIN_PULLUP | PIN_IRQ_POSEDGE,
//	Board_SX_DIO1       | PIN_INPUT_EN | PIN_PULLUP | PIN_IRQ_POSEDGE,
//	Board_SX_DIO2       | PIN_INPUT_EN | PIN_PULLUP | PIN_IRQ_POSEDGE,
//	PIN_TERMINATE
//};
...
void hal_pin_nss (u1_t val){
	PIN_setOutputValue(pinHandle, Board_SX_CSN, val);
}
...
//	}
//
//	if (dioStates[0] != PIN_getInputValue(Board_SX_DIO1)){
...
//void hal_sleep (){
//
//	if((local_targettime-hal_ticks()) < US_PER_OSTICK*400 ) // Increse the *value in case of many Join_Fails
//		return;
//	else
...
//	}
	...
	if (PIN_setOutputValue(pinHandle, Board_SX_CSN, true)  != PIN_SUCCESS) {
		hal_failed();
	}
}
...
//////////////////////////////// SLEEP /////////////////////////////////////////////
void hal_sleep() {
	PIN_setOutputEnable(pinHandle, Board_SX_CSN,      false); // Disable Output
	PIN_setOutputEnable(pinHandle, Board_SX_RF_CTRL1, false); // Disable Output
	PIN_setOutputEnable(pinHandle, Board_SX_RF_CTRL2, false); // Disable Output
	PIN_setOutputEnable(pinHandle, Board_SX_RESET,    false); // Disable Output
	PIN_close(pinHandle);
	pinHandle = PIN_open(&pinState, radioSleepPinTable);
}
